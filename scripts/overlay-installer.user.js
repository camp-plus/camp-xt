...
(function () {
  ...
  // Run in page context: define overlay directly on window if not present.
  try {
    if (window.CAMPOverlay && typeof window.CAMPOverlay === 'function') {
      // Already installed
      window.__CAMP_ready = window.__CAMP_ready || Promise.resolve(window.CAMPOverlay);
      return;
    }

    // Define the overlay as a class expression assigned to window to avoid lexical redeclaration.
    window.CAMPOverlay = class {
  constructor(siteName = 'Site', version = '1.0.3', options = {}) {
  ...
  try { window.CAMPOverlay.__CAMP_VERSION = '1.0.3'; } catch (e) { void e; }
